...
# === ВЕБ-СЕРВЕР (только один экземпляр) ===
...
def run_flask():
    # Используем стандартный порт 8080 для Render
...
# Небольшая задержка перед стартом бота
time.sleep(2)
print("🔄 Запускаем Telegram-бота...")

while True:
    try:
        # Используем non_stop=False и внешний цикл для надежного перезапуска
        bot.polling(non_stop=False, interval=1, timeout=20)
        # Если polling завершился нормально (без исключения), можно выйти или перезапустить
        print("ℹ️ Бот остановлен (polling завершился). Перезапуск...")
        time.sleep(5)
    except Exception as e:
...
# Следующие строки удаляем, они дублируют Flask и содержат вызовы, которые могут привести к ошибке
# print("✅ Достигли места перед keep_alive()")
# def keep_alive(): ...
# keep_alive() # <-- ЭТО НЕ ДОЛЖНО ВЫЗЫВАТЬСЯ
# try: bot.edit_message_text(...) except ... # <-- Это тоже лишнее внизу файла
# print("🛑 Цикл бота завершен.") # <-- Цикл while True должен работать постоянно
